...
class RealTimeComments {
	...
	*/
	...
	public function enqueue_style_n_script( $hook_suffix ) {
		wp_enqueue_script( 'rtc-plugin', plugins_url('js/script.js', __FILE__ ), array('jquery'), false, false );
	...
	function rtc_update() {
		...
		if(!count($new_coms)) {
			// die('{"status":304, "bookmark": '.$this->now.', "msg":"Not modified. Old bookmark '.$bookmark.'"}');    
            die('Ei');
		}
	...
	function rtc_ajaxurl() {
		global $post;
		if(isset($post->ID)) {
			$rv ='<!-- Realtime Comments Updater Plugin: Init -->'.PHP_EOL;
			$rv.='<script type="text/javascript">'.PHP_EOL;
			$rv.='  var ajaxurl = "'.admin_url('admin-ajax.php').'";'.PHP_EOL;
			$rv.='  var rtc_refresh_time = '.$this->refresh.';'.PHP_EOL;
			$rv.='  var rtc_pageload = '.$this->now.';'.PHP_EOL;
			$rv.='  var rtc_bookmark = rtc_pageload;'.PHP_EOL;
			$rv.='  var rtc_postid = '.$post->ID.';'.PHP_EOL;
			$rv.='</script>'.PHP_EOL;
			echo $rv;
		} else {
			echo '<!-- Realtime Comments Updater Plugin: no comments -->'.PHP_EOL;
		}
	}
